...
const SLOW_MOROCCO_SHEET_ID = process.env.SLOW_MOROCCO_SHEET_ID || "1pXP4zRis6vqPFNPEAj0jLe3JU9QpN_isImwY6-wtt04";
...
// Fetch data from Slow Morocco database (for places)
export async function getSlowMoroccoData(tabName: string) {
  const sheets = getGoogleSheetsClient();

  try {
    const response = await sheets.spreadsheets.values.get({
      spreadsheetId: SLOW_MOROCCO_SHEET_ID,
      range: `${tabName}!A1:ZZ`,
    });
    ...
    console.error(`Error fetching Slow Morocco sheet "${tabName}":`, error.message);
    return [];
  }
}
