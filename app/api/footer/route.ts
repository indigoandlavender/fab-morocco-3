...
const SITE_ID = process.env.SITE_ID || "fab-morocco";
...
export async function GET() {
  try {
    ...
    let newsletterConfig = {
      show: true,
      backgroundImage: "",
      title: "The Morocco you haven't seen yet",
    ...
    Object.values(columnMap).forEach((col: any) => {
      ...
      // If this is an "About Morocco" or similar column, ensure "All Stories" is at top
      if (col.title.toLowerCase().includes('about') || col.title.toLowerCase().includes('morocco')) {
        const hasAllStories = col.links.some((l: any) => 
          l.label.toLowerCase() === 'all stories' || l.label.toLowerCase() === 'stories'
        );
        if (!hasAllStories) {
          col.links.unshift({
            order: 0,
    ...
    if (columns.length > 0) {
      ...
      finalColumns = [
        contactColumn,
        {
          ...
          title: "Tours",
          links: [
            { order: 1, label: "All Tours", href: "/tours", type: "link" },
            { order: 2, label: "Day Trips", href: "/day-trips", type: "link" },
            { order: 3, label: "Desert Tours", href: "/tours?focus=desert", type: "link" },
            { order: 4, label: "Custom Tours", href: "/book", type: "link" },
          ],
        },
        {
          number: 3,
          ...
          links: [
            { order: 1, label: "Book Online", href: "/book", type: "link" },
        ...
        },
        {
          ...
          links: [
            ...
            { order: 2, label: "Contact", href: "/contact", type: "link" },
            { order: 3, label: "Visa Info", href: "/visa-info", type: "link" },
        ...
        columns: [
          {
            ...
            title: "Contact",
            links: [
              { order: 1, label: "Marrakech, Morocco", href: null, type: "address" },
              { order: 2, label: "+212 618 070 450", href: "https://wa.me/212618070450", type: "whatsapp" },
            ],
          },
          {
            number: 2,
            ...
            links: [
              { order: 1, label: "All Tours", href: "/tours", type: "link" },
              { order: 2, label: "Day Trips", href: "/day-trips", type: "link" },
              { order: 3, label: "Custom Tours", href: "/book", type: "link" },
            ],
          },
          {
            number: 3,
